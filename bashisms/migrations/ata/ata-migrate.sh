...
source "${BASE_DIR}/bashisms/common/common.sh" 2>/dev/null || true
source "${BASE_DIR}/bashisms/state/state.sh" 2>/dev/null || true
source "${BASE_DIR}/bashisms/recovery/detect.sh" 2>/dev/null || true
source "${BASE_DIR}/bashisms/recovery/core.sh" 2>/dev/null || true

source "${BASE_DIR}/bashisms/migrations/inits/common.sh" 2>/dev/null || true
source "${BASE_DIR}/bashisms/migrations/des/common.sh" 2>/dev/null || true
...
WHAT WILL BE BACKED UP:
  ...
  Everything to /arch-migration-backup-YYYYMMDD-HHMMSS"
    ...
    if [[ "${migration_stage}" == "install" ]]; then
        ...
        local -a base_pkgs=()
        mapfile -t base_pkgs < <(resolve_ata_base_packages)
        retry_command "base system install" \
            _pacman -S --noconfirm "${base_pkgs[@]}"
        ...
        local init_pkgs=""
        init_pkgs="$(resolve_init_fallback_packages "${target_init}")"
        [[ -n "$init_pkgs" ]] && critical_pkgs+=($init_pkgs)

        local net_stack
        net_stack="$(state_get NETWORK_STACK networkmanager)"
        local net_pkgs=""
        net_pkgs="$(resolve_network_packages "${net_stack}" "${target_init}")"
        [[ -n "$net_pkgs" ]] && critical_pkgs+=($net_pkgs)

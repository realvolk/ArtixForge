...
stage_post() {
    ...
    for key in "${STATE_KEYS_CHROOT[@]}"; do
        env_exports+="export ${key}=\"$(state_get "${key}" "${STATE_DEFAULTS[$key]:-}")\""$'\n'
    done
...
source ./bashisms/installer/post/drivers.sh
source ./bashisms/installer/post/networking.sh
source ./bashisms/installer/post/desktop.sh
source ./bashisms/installer/post/audio.sh
source ./bashisms/installer/post/extras.sh
